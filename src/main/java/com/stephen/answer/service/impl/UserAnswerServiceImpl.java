...
import com.stephen.answer.mapper.UserAnswerMapper;
...
public class UserAnswerServiceImpl extends ServiceImpl<UserAnswerMapper, UserAnswer> implements UserAnswerService {
	...
	public void validUserAnswer(UserAnswer userAnswer, boolean add) {
		...
		if (add) {
			// todo 补充校验规则
			ThrowUtils.throwIf(appId == null || appId <= 0, ErrorCode.PARAMS_ERROR, "评分策略类型错误");
		}
		// 修改数据时，有参数则校验
		// todo 补充校验规则
		if (appId != null) {
			...
			ThrowUtils.throwIf(!Objects.equals(ReviewStatusEnum.getEnumByValue(app.getReviewStatus()), ReviewStatusEnum.PASS), ErrorCode.PARAMS_ERROR, "应用未过审");
		}
	}
	 ...
	 * @return QueryWrapper<UserAnswer>
	 */
	@Override
